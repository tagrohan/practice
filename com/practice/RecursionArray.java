...
public class RecursionArray {
   public static void main(String[] args) {
      ...
      System.out.println(Arrays.toString(findAllIndex(new int[]{1, 8, 5, 6, 8, 3, 8}, 0, 8, 0)));

   }

   // fsf = found so far (last video in array recursion pep.coding)
   private static int[] findAllIndex(int[] arr, int i, int key, int fsf) {
      ...
      if (arr[i] == key) {
         int[] whileTesting = findAllIndex(arr, i + 1, key, fsf + 1);
         whileTesting[fsf] = i;
         return whileTesting;
      } else {
         return findAllIndex(arr, i + 1, key, fsf);
      }
   }

   // finding all indices of the searched element in array
